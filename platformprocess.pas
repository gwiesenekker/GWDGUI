...
type
  ...
  public
    constructor Create(ASlotIndex: Integer);
...
constructor TPlatformProcess.Create(ASlotIndex: Integer);
begin
  inherited Create;
...
begin
  ...
  try
    ...
    FRunning := True;
    FReaderThread := TPlatformProcessReaderThread.Create(Self, FOutputReadHandle);
  finally
...
procedure TPlatformProcess.RequestQuit(const AQuitCommand: String;
  AWaitMs: Integer);
begin
  if not IsRunning then
    Exit;
